...
import { PlayIcon, PauseIcon, SkipForwardIcon, SkipBackIcon, PlaylistIcon, WarningIcon, SpinnerBallIcon } from '@phosphor-icons/react';
      ...
      <div className="w-full max-w-screen-xl mx-auto px-4 py-3 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 truncate text-sm font-medium">
            {loading ? <SpinnerBallIcon className="w-5 h-5" size={18} /> : error ? <WarningIcon className="w-5 h-5" size={18} /> : currentTrack?.title ?? 'No song loaded'}
          </div>
            ...
            <button onClick={() => setShowPlaylist((v) => !v)}>
              <PlaylistIcon
                className={`w-5 h-5 transition-transform ${
                  showPlaylist ? 'rotate-90 text-blue-900' : ''
                }`}
              />
            </button>
          </div>
